...
- 恢复 `MIN_MATCHES_FOR_FULL_QUOTA = 6` 告警语义。
...
- 方案胜负标签改为主胜 / 客胜。
- 比赛列表每场参考槽位去掉「参考」前缀，只保留方向和概率。
- 每场参考只在方向与本行已展示的玩法一致时才着色，避免标签与比分互相矛盾。
- 参考比分统一走 `_score_hint_text` 出文案并去重，不再出现「比分:3-1/3-1」。
- 日推一致性闸补齐大小球与双进校验：比分结算不了展示中的这两项就返回 None 交给降级，不再发出「双进:是 · 小(3) · 比分 2-0」。
- 大小球 / 双进注的伴随胜负行改为可让步（最可能结果 → 和局 → 另一侧），不再因伴随行讲不圆而淘汰概率最高的那一注；和局时让球行指向受让方。
- 分析器双进方向改以双进盘口为准（原先只按大小球形态与 1X2 均衡度打分，1282 场留出段 52.1% 对 58.3%），卡片双进标签与每场参考不再经常反向。
- 投注栏展开按钮由箭头图标改为「详情」文字。
- 比赛列表、关注与赛果卡片不再显示推荐强度（星级与百分比均移除）；本场最看好的玩法只靠 `[荐]` 与绿色标签表达。`favorite_fixtures.confidence` 及接口上只为该展示存在的参考 EV / 来源 / 一致性 / 原因字段一并删除，命中概率仍在 `auto_pick_snapshots` 供审计。
- 手机端点推荐弹出的赛前盘口弹窗去掉标题栏与关闭按钮（点空白即关），表格留白压紧、列宽收窄，390px 下不再裁掉客队列。
- 详情「我的预测」盘口快照：初盘说明只留「首次采集的机构盘口」，中盘 / 临场 / 即时盘不再写阶段说明；时间改为 `09-20 10:55`，去掉「采集」和时区标注。

## 2026-09-23（周三）

- 打开订阅不再自动开启密刷，只解禁该开关；关闭订阅仍强制关闭密刷。密刷默认值改为关。
- 盘口快照表双进改回独立一行，不再与大小球合并。

## 2026-09-24（周四）

- 女足欧冠俱乐部与 9/23 新入库赛程中缺中文的球队补了译名（含友谊赛青年队、巴拉圭杯等当日新场）；补上罗马尼亚U19。
- 非洲杯预选赛国家队补了中文译名，赛程入库后可直接显示。
- 澳超俱乐部补了中文译名，10 月开赛入库后可直接显示。
- 两球及以上让球盘若让球方赔率低于 1.60，日推降级到大小球 / 双进或后续场次。
- 官方把比赛改到别的日子后，本地旧赛程行不再占【比赛】：当天赛程同步时对账，清单里消失的未开赛场次标为延期，官方改到新日期后自动复位。
- 详情【统计】的历史交锋与近期战绩表改成四列（日期/赛事、对阵、让球、总进球），每格双行：对阵下方是半场比分，让球与总进球未开赛时下方显示初盘、完赛后显示按关注球队视角的输赢结果。
- 恢复【统计】表格里丢失的胜负着色：历史交锋按本场主队视角、近期战绩按该栏球队视角，给队名与它那一侧的进球数上胜红 / 平灰 / 负蓝。
- 【赛前分析】盘口阶段标题改用次级文本色；API-Sports 建议改为中文，并恢复包含泊松分布的官方队际对比表。官方对无预测覆盖的场次会回「No predictions available」加 33%/33%/33%，这种占位不再当成预测展示，直接显示空态。
- 详情只留【统计】和【赛前分析】。【统计】为历史交锋、近期战绩、伤病（不再展示阵容）；【赛前分析】顺序为四阶段盘口、预测对比、双方近 5 场数据对比、API-Sports 官方建议表。
