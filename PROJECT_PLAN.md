...
| 官方数据拉取           | 🟡 部分完成 | 联赛、球队、赛程、H2H、近况、球队统计已接；赔率/阵容/伤病未接           |
...
| 前端展示             | 🟡 MVP  | 联赛 → 今日赛程 → 概率图                             |
| 赛前详情页（赔率/阵容/伤病等） | ❌ 未做    | 表字段部分预留                                     |
| 官方 Widgets       | ❌ 未做    | 可选增强，非必须                                    |
| 测试 / 部署          | ❌ 未做    | 无自动化测试与生产部署方案                               |

**整体判断：MVP 可演示（联赛 + 今日赛程 + 基础概率分析）；距完整「赛前分析产品」仍差赔率/阵容/伤病等数据闭环与前端呈现。**
...
| 需求     | 现状                                       | 缺口                        |
|--------|------------------------------------------|---------------------------|
| 赛前赔率   | 未接                                       | 拉取 `/odds`、落库、API 暴露、前端展示 |
| 阵容（首发） | `home_formation` / `away_formation` 字段已有 | 未拉取、未解析 XI、未展示            |
| 替补     | 无                                        | 模型 + 拉取 + 展示              |
| 伤病     | `injuries_*` 字段已有                        | 未拉取、未展示                   |
| 历史交锋展示 | 已拉取并缓存                                   | 未进入分析公式，未进 API/UI         |
| 近况展示   | 已用于概率                                    | 未在详情页结构化展示                |
| 球队统计   | 已拉取                                      | 未进入公式，未展示                 |
...
| M4 赛前数据包   | 赔率 + 阵容 + 伤病 + 替补 + H2H 展示闭环 | ⬜ 下一目标 |
