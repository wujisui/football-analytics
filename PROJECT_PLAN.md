...
前端：始终只调本项目后端。列表页、F5、下拉刷新、切换筛选只读本地库；进入详情走 `/fixtures/{id}/analysis`。未订阅时详情完全只读本地公共数据；已订阅时热门联赛缺包可按需打官方并落库复用。已移除「定时全量获取详情」独立开关，能力收敛到订阅状态。每次定时同步盘口后，对**已有赛前盘口的未开赛场次**按后端持久化的**比赛所在地日历日**各自做**单选价值排序**：先用冻结历史按玩法做时间留出 Platt 概率校准（最近 20% 只验证，Brier 与 log-loss 未同时改善的玩法保持原概率），再以校准概率计算期望收益；历史命中反馈只参与综合分调整、不设置历史分位淘汰门槛，每天按综合分最多选 4 场写入自动推荐池（`source=auto`，只替换自动推荐）。入选后在同一比赛日内部按综合分评级，最高质量锚定 5 星，其余每降一个得分档位减 0.5 星，同分同星。自动推荐只用于【比赛】/【赛程】置顶、`[荐]` 与质量星展示，不点亮关注星，也不进入【关注】；【关注】只展示用户点亮星标并经收藏接口写入数据库的 `source=manual` 记录。校准只改变排序、不减少每日数量；拒绝胜/平、让胜/负等多选；精确比分因无独立校准概率且历史命中率低，只作详情提示，不参与自动主推；窗口内有多少个有赛日就有多少组推荐，禁止把整周场次压成全局 Top4。不做前端轮询或 SSE。**让球胜平负的展示与结算**只在「我的 → 偏好设置」用同一开关切换：默认**亚盘**（让胜/让负；整数盘打成让球平走水，四分盘出赢半/输半）；**竞彩**先把盘口按绝对值向上取整成整数（-0.25/-0.5/-0.75 → -1，-1.25~-1.75 → -2）再判让胜/让平/让负，不出半结果，展示的盘口同步显示取整后的线。口径只作用于让球胜平负，大小球、双进等其余玩法一律按亚盘拆盘。只改本机展示与结算，不回写冻结预测或已存方案；机器学习与每日推荐激励仍按产品默认亚盘。真源：`ah_features.jc_handicap_line`。
自动推荐按产品默认亚盘筛选：除多选外，单独「让平」只代表退本走水、不是可投注方向，也不得写入自动推荐池；竞彩开关只改变展示与结算，不另起一套推荐排序。
...
| 同步后自动推荐 | 未开赛且已有盘口的场次先按玩法做时间留出 Platt 校准，再按校准概率 × 赔率的期望收益与历史软权重排序；按后端持久化的**比赛所在地日历日**每天最多 4 场写入自动推荐池（`source=auto`），数量策略不变；不是整周全局 Top4。自动推荐在【比赛】/【赛程】置顶并显示 `[荐]`/质量星，但不点亮关注星且不进入【关注】；【关注】只展示 `source=manual` 的接口落库记录 |
