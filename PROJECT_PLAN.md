...
开赛后：预测快照审计字段冻结；详情展示包可在用户点开时按需补缺并落库；未完场比赛打开详情时补一次比分（开赛 12 小时内 60 秒缓存；此后到 7 天内仍未结算的按 30 分钟缓存补一次终场，不轮询）
赛果：六个固定同步批次回写终场比分（非滚球）
前端：始终只调本项目后端。列表页、F5、下拉刷新、切换筛选只读本地库；进入详情走 `/fixtures/{id}/analysis`（后端本地优先，缺包时可打官方）。定时批次按 `SCHEDULER_TIMEZONE` 每天 **00:00、06:00、11:00、16:00、19:00、22:00** 同步赛程、盘口、赛果，并对窗口内**有比赛的联赛**按「联赛+赛季」各拉一次积分榜（同一自然日已拉过则跳过）；列表排名只读本地联赛积分榜快照。每次定时同步盘口后，对热门联赛（`config/leagues.json`）未开赛场次按 **UTC 比赛日** 各自做**单选价值排序**，历史命中反馈只参与综合分调整、不设置历史分位淘汰门槛，每天按综合分最多选 4 场写入「关注」（`source=auto`，只替换自动关注）；入选后在同一比赛日内部按综合分评级，最高质量锚定 5 星，其余每降一个得分档位减 0.5 星，同分同星。拒绝胜/平、让胜/负等多选；精确比分因无独立校准概率且历史命中率低，只作详情提示，不参与自动主推；窗口内有多少个有赛日就有多少组推荐，禁止把整周场次压成全局 Top4。不做前端轮询或 SSE；不做定时全量详情预拉（默认关，管理员可在「我的 → 管理员设置」开启并落库）
...
| 同步后自动关注 | 热门联赛未开赛场次按 **UTC 比赛日** 各自单选价值排序，每天最多 4 场写入关注（`source=auto`）；不是整周全局 Top4 |
| 勾选其他联赛 | 只筛选数据库中已有赛程，不触发官方盘口请求 |
| 用户打开详情 | 本地缺展示包时按需补拉阵容/伤病/交锋/近况/简报等并落库；积分榜优先读联赛快照，缺时再补拉并回写快照；已开赛未完场额外补一次比分（12 小时内 60 秒缓存跟比分，12 小时~7 天按 30 分钟缓存追终场结算） |
