...
联赛范围：赛程按日**全量**入库；`backend/config/leagues.json` 为一级联赛默认勾选/盘口刷新范围；当日有赛但未列入该目录的联赛进入筛选框（`extra`），勾选只影响本地显示
```

| 时点 | 盘口与推荐刷新策略 |
|---|---|
| 六个固定同步批次 | 全量赛程、一级联赛已有/缺失盘口与赛果统一更新；首次可用盘口冻为初盘；窗口内有赛联赛积分榜按日各拉一次；同步后刷新自动关注 |
