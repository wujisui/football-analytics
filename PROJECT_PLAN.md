...
| 与 1X2 关系 | **独立模型**，但展示倾向不得与同卡片的胜平负推荐互斥：`|让球线| ≤ 0.5` 时结算完全由胜平负结果决定，若某一侧会在推荐结果上**输掉全注**（胜/平 配 让负(-0.25)、负/平 配 让胜(+0.25)），改取镜像侧并在 `handicap_market_note` 说明；平局在四分盘只输半/赢半，不构成互斥。`|让球线| > 0.5` 由净胜球决定，不做该约束。真源 `ah_predictor._agree_with_recommendation` |
