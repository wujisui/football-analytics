...
| 需求     | 现状                                       | 缺口                        |
|--------|------------------------------------------|---------------------------|
| 赛前赔率   | 固定批次首次可用盘口冻为**初盘**（`odds_opening_json`），后续批次写**即时盘**（`odds_json`）并重算 1X2。单份盘口只取一家庄（`BOOKMAKER_PRIORITY` 中覆盖核心玩法最多者，只有主庄没开的玩法才回退次级庄）；初盘若冻在次级庄，主庄开盘后按主庄首盘替换，开赛后不再改写 | 官方未开盘仍空态 |
