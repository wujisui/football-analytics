...
```
开赛前：定时同步批次刷新赛程与一级联赛盘口，首次可用盘口冻结为初盘（默认免费配额模式每天 **11:00** 全量 + **22:00** 盘口轻刷，且只拉今天；关闭后恢复未来窗口与六个整点）
...
免费配额范围：官方请求固定为「昨天赛果 + 今天比赛/盘口」，**跳过积分榜**（防止积分榜逐联赛拉取耗光当日配额、导致赛程/盘口不完整；排名读上一次本地快照），不请求未来比赛；关闭免费配额后才恢复配置的未来窗口与积分榜
前端：始终只调本项目后端。列表页、F5、下拉刷新、切换筛选只读本地库；进入详情走 `/fixtures/{id}/analysis`（后端本地优先，缺包时可打官方）。定时批次按 `SCHEDULER_TIMEZONE` 同步赛程、盘口、近几日赛果，并对窗口内**有比赛的联赛**按「联赛+赛季」各拉一次积分榜（同一自然日已拉过则跳过）。**默认开启免费配额模式**（管理员「我的 → 管理员设置 → 打开免费配额」）：每天跑 **11:00**（昨天赛果 + 今天赛程/盘口）与 **22:00**（只刷新今天热门联赛盘口并重算每日推荐）；关闭后恢复 **00:00、06:00、11:00、16:00、19:00、22:00**。列表排名只读本地联赛积分榜快照。每次定时同步盘口后，对热门联赛（`config/leagues.json`）未开赛场次按后端持久化的**比赛所在地日历日**各自做**单选价值排序**：先用冻结历史按玩法做时间留出 Platt 概率校准（最近 20% 只验证，Brier 与 log-loss 未同时改善的玩法保持原概率），再以校准概率计算期望收益；历史命中反馈只参与综合分调整、不设置历史分位淘汰门槛，每天按综合分最多选 4 场写入「关注」（`source=auto`，只替换自动关注）。入选后在同一比赛日内部按综合分评级，最高质量锚定 5 星，其余每降一个得分档位减 0.5 星，同分同星。校准只改变排序、不减少每日数量；拒绝胜/平、让胜/负等多选；精确比分因无独立校准概率且历史命中率低，只作详情提示，不参与自动主推；窗口内有多少个有赛日就有多少组推荐，禁止把整周场次压成全局 Top4。不做前端轮询或 SSE；不做定时全量详情预拉（默认关，管理员可在「我的 → 管理员设置」开启并落库）。**让球胜平负的展示与结算**用桌面顶栏开关切换（「我的 → 主题与玩法」只展示口径说明）：默认**亚盘**（让胜/让负；整数盘打平为走水，四分盘拆盘出赢半/输半）；**竞彩**先把盘口按绝对值向上取整成整数（-0.25/-0.5/-0.75 → -1，-1.25~-1.75 → -2）再判让胜/让平/让负，不出半结果，展示的盘口同步显示取整后的线。口径只作用于让球胜平负，大小球、双进等其余玩法一律按亚盘拆盘。只改本机展示与结算，不回写冻结预测或已存方案；机器学习与每日推荐激励仍按产品默认亚盘。真源：`ah_features.jc_handicap_line`。

联赛范围：官方按日赛程响应只入库**赛事白名单**：`backend/config/leagues.json` 为热门赛事（默认勾选/盘口刷新范围）；“其他”只保留已核对 ID 的各国顶级联赛、主要足球地区高级别杯赛，以及俱乐部/国际友谊赛，白名单真源为 `competition_scope.py`，新增赛事需显式核对 ID 后加入。青年、预备队、乙级及以下和低级别杯赛不入库；当日有赛且在白名单、但未列入热门目录的赛事进入筛选框（`extra`），勾选只影响本地显示
```

| 时点 | 盘口与推荐刷新策略 |
|---|---|
| 六个固定同步批次 | 白名单赛程、热门联赛已有/缺失盘口与赛果统一更新；首次可用盘口冻为初盘；窗口内有赛联赛积分榜按日各拉一次；同步后刷新自动关注 |
| 同步后自动关注 | 热门联赛未开赛场次先按玩法做时间留出 Platt 校准，再按校准概率 × 赔率的期望收益与历史软权重排序；按后端持久化的**比赛所在地日历日**每天最多 4 场写入关注（`source=auto`），数量策略不变；不是整周全局 Top4 |
| 勾选其他联赛 | 只筛选数据库中已有赛程，不触发官方盘口请求 |
| 用户打开详情（未开赛 / 从未分析过） | 本地缺展示包时按需补拉阵容/伤病/交锋/近况/简报等并落库；积分榜优先读联赛快照，缺时再补拉并回写快照 |
