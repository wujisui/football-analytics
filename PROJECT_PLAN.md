...
前端：始终只调本项目后端。列表页、F5、下拉刷新、切换筛选只读本地库；进入详情走 `/fixtures/{id}/analysis`。未订阅时详情完全只读本地公共数据；已订阅时热门联赛缺包可按需打官方并落库复用。已移除「定时全量获取详情」独立开关，能力收敛到订阅状态。每次定时同步盘口后，对**已有赛前盘口的未开赛场次**按后端持久化的**比赛所在地日历日**各自做**单选价值排序**：先用冻结历史按玩法做时间留出 Platt 概率校准（最近 20% 只验证，Brier 与 log-loss 未同时改善的玩法保持原概率），再以校准概率计算期望收益；历史命中反馈只参与综合分调整、不设置历史分位淘汰门槛，每天按综合分最多选 4 场写入自动推荐池（`source=auto`，只替换自动推荐）。入选后在同一比赛日内部按综合分评级，最高质量锚定 5 星，其余每降一个得分档位减 0.5 星，同分同星。自动推荐只用于【比赛】/【赛程】置顶、`[荐]` 与质量星展示，不点亮关注星，也不进入【关注】；【关注】只展示用户点亮星标并经收藏接口写入数据库的 `source=manual` 记录。校准只改变排序、不减少每日数量；拒绝胜/平、让胜/负等多选；精确比分因无独立校准概率且历史命中率低，只作详情提示，不参与自动主推；窗口内有多少个有赛日就有多少组推荐，禁止把整周场次压成全局 Top4。不做前端轮询或 SSE。**让球胜平负的展示与结算**在桌面顶栏、手机「我的 → 主题与玩法」使用同一开关切换：默认**亚盘**（让胜/让负；整数盘打成让球平走水，四分盘出赢半/输半）；**竞彩**先把盘口按绝对值向上取整成整数（-0.25/-0.5/-0.75 → -1，-1.25~-1.75 → -2）再判让胜/让平/让负，不出半结果，展示的盘口同步显示取整后的线。口径只作用于让球胜平负，大小球、双进等其余玩法一律按亚盘拆盘。只改本机展示与结算，不回写冻结预测或已存方案；机器学习与每日推荐激励仍按产品默认亚盘。真源：`ah_features.jc_handicap_line`。

联赛范围：官方按日赛程响应只入库**赛事白名单**：`backend/config/leagues.json` 是可勾选的完整目录（修改后需重启后端），**不**随页面改写。侧栏「热门」与定时盘口范围由管理员「我的 → 热门联赛」勾选，落库 `app_settings.hot_league_ids`；未落库时默认五大联赛 + 欧冠/欧罗巴/欧协联 + 中超/日职联/韩K联。未勾选的目录联赛及白名单外当日有赛项进入「其他」。白名单真源为 `competition_scope.py`，只保留已核对 ID 的各国顶级联赛、主要足球地区高级别杯赛，以及俱乐部/国际友谊赛；青年、预备队、乙级及以下和低级别杯赛不入库。筛选弹窗里热门默认勾选，切换日期后仍默认勾选（只有用户点过确认的选择才落 localStorage，加载时推导的勾选不落库）；筛选勾选只影响本地显示；非热门不打官方盘口（定时与详情均不补盘）。已开赛详情只补比分与状态。
