...
| 同步后自动推荐 | 未开赛且已有盘口的场次先按玩法做时间留出 Platt 校准；`±0.5` 等价结算事件统一概率并按实际赔率择优，其余模型优势向市场公平概率保守收缩，再以凯利型风险收益分叠加历史软权重排序；按后端持久化的**比赛所在地日历日**每天最多 4 场写入自动推荐池（`source=auto`），数量策略不变；不是整周全局 Top4。自动推荐在【比赛】/【赛程】置顶并显示 `[荐]`/质量星，但不点亮关注星且不进入【关注】；【关注】只展示 `source=manual` 的接口落库记录 |
