...
前端：始终只调本项目后端。列表页、F5、下拉刷新、切换筛选只读本地库；进入详情走 `/fixtures/{id}/analysis`（后端本地优先，缺包时可打官方）。定时批次按 `SCHEDULER_TIMEZONE` 每天 **00:00、06:00、11:00、16:00、19:00、22:00** 只同步赛程、盘口与赛果；不做前端轮询或 SSE；不做定时全量详情预拉（默认关，管理员可在「我的 → 管理员设置」开启并落库）
...
| 时点 | 盘口与推荐刷新策略 |
|---|---|
| 六个固定同步批次 | 全量赛程、一级联赛已有/缺失盘口与赛果统一更新；首次可用盘口冻为初盘 |
| 勾选其他联赛 | 只筛选数据库中已有赛程，不触发官方盘口请求 |
| 用户打开详情 | 本地缺展示包时按需补拉阵容/伤病/交锋/近况/简报等并落库 |
...
| 定时任务             | ✅ 完成    | **每日 00:00 / 06:00 / 11:00 / 16:00 / 19:00 / 22:00** 统一同步赛程、盘口、赛果（含自动 train） |
