...
未订阅范围：官方请求固定为「00:05 UTC（北京时间 08:05）当天赛程 + 11:00 昨天赛果/今天比赛/热门盘口 + 22:00 今天热门盘口」。08:05 只多一次全球 `fixtures?date=` 请求，**不拉盘口、积分榜或详情**；11:00 跳过积分榜与详情预拉、不请求未来；详情点击完全只读本地。当天完整批次尚未成功时可由管理员「立即同步」补跑，成功后按钮禁用；单场盘口只能在详情「我的预测」点击「更新盘口」
已订阅范围：每天 11:00（或当天尚未成功时的「立即同步」）跑唯一完整批次。赛程在本地始终保留今天起 8 天，但每天只增量拉窗口末端新一天，不重复请求重叠的未来 7 天；近 4 天赛果仍按日回写。盘口与详情只处理今天、明天：明天首次可用盘口写初盘，第二天成为今天后更新即时盘；详情预拉只覆盖今天、明天热门未开赛缺包，打开详情仍可按需补整包。11:55 及 14/16/18/20/22/00/02 只刷新今天未开赛热门盘口并重算日推；04/06/08/10 由默认开启的独立开关控制。当天完整批次成功后「立即同步」禁用
