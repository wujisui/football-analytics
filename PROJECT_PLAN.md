...
前端：始终只调本项目后端。列表页、F5、下拉刷新、切换筛选只读本地库；进入详情走 `/fixtures/{id}/analysis`（后端本地优先，缺包时可打官方）。定时批次按 `SCHEDULER_TIMEZONE` 每天 **00:00、06:00、11:00、16:00、19:00、22:00** 同步赛程、盘口、赛果，并对窗口内**有比赛的联赛**按「联赛+赛季」各拉一次积分榜（同一自然日已拉过则跳过）；列表排名只读本地联赛积分榜快照。每次定时同步盘口后，从热门联赛（`config/leagues.json`）未开赛场次中按**单选价值排序**挑出 4 场写入「关注」（`source=auto`，只替换自动关注，不碰手动关注）：拒绝胜/平、让胜/负等多选；赔率与期望收益只用于相对排序（优先更好回报，但仍是矮子里拔高个），场次够则必须 4 场，仅当可推荐场次不足时才少选。不做前端轮询或 SSE；不做定时全量详情预拉（默认关，管理员可在「我的 → 管理员设置」开启并落库）
...
| 同步后自动关注 | 热门联赛未开赛场次按**单选价值排序**取 4 场（场次够必须满额）；赔率只影响排序、不硬砍到 0，写入关注（`source=auto`） |
