...
```
开赛前：定时同步批次只为管理员勾选的热门联赛刷新盘口，首次可用盘口冻结为初盘（默认免费配额模式每天官方 UTC 换日后 **00:05 UTC / 08:05 北京时间**只同步当天赛程、**11:00** 全量 + **22:00** 盘口轻刷；08:05 不拉盘口，关闭后恢复未来窗口与六个整点）
...
免费配额范围：官方请求固定为「00:05 UTC（北京时间 08:05）当天赛程 + 11:00 昨天赛果/今天比赛/热门盘口 + 22:00 今天热门盘口」。08:05 只多一次全球 `fixtures?date=` 请求，**不拉盘口、积分榜或详情**；其余批次继续**跳过积分榜与全部详情获取**（防止逐联赛/逐场请求耗光当日配额），不请求未来比赛；详情点击完全只读本地，连缺失盘口也不按点击补拉；管理员已开启的定时全量详情设置在免费配额期间暂停。关闭免费配额后才恢复配置的未来窗口、积分榜与热门联赛全量详情按需获取
前端：始终只调本项目后端。列表页、F5、下拉刷新、切换筛选只读本地库；进入详情走 `/fixtures/{id}/analysis`。免费配额开启时详情完全只读本地公共数据；关闭后，热门联赛缺包才可按需打官方并落库复用。定时批次按 `SCHEDULER_TIMEZONE` 同步赛程、盘口、近几日赛果，并对窗口内**有比赛的热门联赛**按「联赛+赛季」各拉一次积分榜（同一自然日已拉过则跳过）。**默认开启免费配额模式**（管理员「我的 → 管理员设置 → 打开免费配额」）：每天跑 **11:00**（昨天赛果 + 今天赛程/盘口）与 **22:00**（只刷新今天热门联赛盘口并重算每日推荐）；关闭后恢复 **00:00、06:00、11:00、16:00、19:00、22:00**。列表排名只读本地联赛积分榜快照。每次定时同步盘口后，对**已有赛前盘口的未开赛场次**按后端持久化的**比赛所在地日历日**各自做**单选价值排序**：先用冻结历史按玩法做时间留出 Platt 概率校准（最近 20% 只验证，Brier 与 log-loss 未同时改善的玩法保持原概率），再以校准概率计算期望收益；历史命中反馈只参与综合分调整、不设置历史分位淘汰门槛，每天按综合分最多选 4 场写入自动推荐池（`source=auto`，只替换自动推荐）。入选后在同一比赛日内部按综合分评级，最高质量锚定 5 星，其余每降一个得分档位减 0.5 星，同分同星。自动推荐只用于【比赛】/【赛程】置顶、`[荐]` 与质量星展示，不点亮关注星，也不进入【关注】；【关注】只展示用户点亮星标并经收藏接口写入数据库的 `source=manual` 记录。校准只改变排序、不减少每日数量；拒绝胜/平、让胜/负等多选；精确比分因无独立校准概率且历史命中率低，只作详情提示，不参与自动主推；窗口内有多少个有赛日就有多少组推荐，禁止把整周场次压成全局 Top4。不做前端轮询或 SSE；定时全量详情默认关，管理员可在「我的 → 管理员设置」开启并落库，但仅在免费配额关闭时生效。**让球胜平负的展示与结算**在桌面顶栏、手机「我的 → 主题与玩法」使用同一开关切换：默认**亚盘**（让胜/让负；整数盘打成让球平走水，四分盘出赢半/输半）；**竞彩**先把盘口按绝对值向上取整成整数（-0.25/-0.5/-0.75 → -1，-1.25~-1.75 → -2）再判让胜/让平/让负，不出半结果，展示的盘口同步显示取整后的线。口径只作用于让球胜平负，大小球、双进等其余玩法一律按亚盘拆盘。只改本机展示与结算，不回写冻结预测或已存方案；机器学习与每日推荐激励仍按产品默认亚盘。真源：`ah_features.jc_handicap_line`。
...
| 时点 | 盘口与推荐刷新策略 |
|---|---|
| 六个固定同步批次 | 白名单赛程、热门勾选联赛已有/缺失盘口与赛果统一更新；首次可用盘口冻为初盘；窗口内有赛的热门联赛积分榜按日各拉一次；同步后刷新自动推荐 |
