...
前端：始终只调本项目后端。列表页、F5、下拉刷新、切换筛选只读本地库；进入详情走 `/fixtures/{id}/analysis`（后端本地优先，缺包时可打官方）。定时批次按 `SCHEDULER_TIMEZONE` 同步赛程、盘口、近几日赛果，并对窗口内**有比赛的联赛**按「联赛+赛季」各拉一次积分榜（同一自然日已拉过则跳过）。**默认开启免费配额模式**（管理员「我的 → 管理员设置 → 打开免费配额」）：每天只跑 **11:00**；关闭后恢复 **00:00、06:00、11:00、16:00、19:00、22:00**。列表排名只读本地联赛积分榜快照。每次定时同步盘口后，对热门联赛（`config/leagues.json`）未开赛场次按 **UTC 比赛日** 各自做**单选价值排序**：先用冻结历史按玩法做时间留出 Platt 概率校准（最近 20% 只验证，Brier 与 log-loss 未同时改善的玩法保持原概率），再以校准概率计算期望收益；历史命中反馈只参与综合分调整、不设置历史分位淘汰门槛，每天按综合分最多选 4 场写入「关注」（`source=auto`，只替换自动关注）。入选后在同一比赛日内部按综合分评级，最高质量锚定 5 星，其余每降一个得分档位减 0.5 星，同分同星。校准只改变排序、不减少每日数量；拒绝胜/平、让胜/负等多选；精确比分因无独立校准概率且历史命中率低，只作详情提示，不参与自动主推；窗口内有多少个有赛日就有多少组推荐，禁止把整周场次压成全局 Top4。不做前端轮询或 SSE；不做定时全量详情预拉（默认关，管理员可在「我的 → 管理员设置」开启并落库）
...
| 同步后自动关注 | 热门联赛未开赛场次先按玩法做时间留出 Platt 校准，再按校准概率 × 赔率的期望收益与历史软权重排序；按 **UTC 比赛日** 每天最多 4 场写入关注（`source=auto`），数量策略不变；不是整周全局 Top4 |
