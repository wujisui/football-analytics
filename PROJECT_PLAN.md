...
| 同步后自动推荐 | 未开赛且已有盘口的场次先按玩法做时间留出 Platt 校准；`±0.5` 等价结算事件统一概率并按实际赔率择优，其余模型优势向市场公平概率保守收缩，再以凯利型风险收益分叠加历史软权重排序；按后端持久化的**比赛所在地日历日**每天最多 4 场写入自动推荐池（`source=auto`），数量策略不变；不是整周全局 Top4。自动推荐在【比赛】/【赛程】置顶并显示 `[荐]`/质量星，但不点亮关注星且不进入【关注】；【关注】只展示 `source=manual` 的接口落库记录。被日推选中的卡片整行展示日推三件套（`auto_lean` / `auto_handicap_lean` / `auto_score_hint`），不与分析器的双选文案混排 |
...
| 与 1X2 关系 | **独立模型**。普通详情展示由 `ah_predictor._agree_with_recommendation` 约束 `|让球线| ≤ 0.5`；日推在 Top-N 前以**自己的单选方向**为唯一基准产出自洽三件套：真实盘口上该方向不会输的让球侧，加上按该方向重算、沿用本场大小球/双进结论的比分候选。主胜遇深于主让 1 球、平局遇 `|线| > 0.25`、平手盘遇平局等无法表达的场次跳过并由后续候选补位，绝不改盘口线。三件套只落 `favorite_fixtures` 与 `auto_pick_snapshots`，不回写 `pre_match_data`。真源 `recommendation.consistency.validate_pick_consistency` |
