...
前端：始终只调本项目后端；**刷新页面**即触发一次 `POST /fixtures/sync`（同步等待落库后返回），页面先展示本地库内容并标注「正在从官方同步…」，落库后自动更新；同步进行中重复刷新一律忽略，不重复打官方

联赛范围：赛程按日**全量**入库；`backend/config/leagues.json` 为一级联赛默认勾选/盘口补全范围；当日有赛但未列入该目录的联赛出现在筛选勾选框（`extra`），勾选后才补盘口
```

| 时点 | 盘口与推荐刷新策略 |
|---|---|
| 赛程同步 | 只补缺失盘口；首次可用盘口冻为初盘并完成初始分析 |
| 勾选其他联赛 | 当日有赛但不在 `leagues.json` 的联赛；勾选后按 `odds_only` 补拉其盘口 |
...
- **公共池**（平台）：定时赛程/初盘/一级联赛缺盘/临场 30 分钟/按日赛果 —— 一人拉、多人读，不按用户计次。
- **用户池**（登录/VIP）：手动 sync、非一级联赛补盘、详情按需补拉、PC「刷新官方」等 —— 按动作扣日额度；用尽后只读本地。
...
- [x] `scheduled_fixtures_sync`：每日 06:00 / 12:00 / 18:00 全量拉取日期条未来窗口赛程 + 一级联赛缺盘补全；首次盘口冻为初盘，手动同步为即时盘
...
| M6-U 用户/VIP | 真登录 + 用户池额度 + 手动 sync 计量（见 [docs/AUTH_VIP_QUOTA.md](docs/AUTH_VIP_QUOTA.md)） | 📋 设计已写入，代码未开 |
...
| 风险           | 应对                                   |
|--------------|--------------------------------------|
| 官方 API 日配额有限 | 本地优先 + 开赛后冻结 + 按开赛时间刷新；中长期见 [docs/AUTH_VIP_QUOTA.md](docs/AUTH_VIP_QUOTA.md)（公共池/用户池） |
