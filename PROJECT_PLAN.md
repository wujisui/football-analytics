...
前端：始终只调本项目后端。列表页、F5、下拉刷新、切换筛选只读本地库；进入详情走 `/fixtures/{id}/analysis`。未订阅时详情完全只读本地公共数据；已订阅时热门联赛缺包可按需打官方并落库复用。已移除「定时全量获取详情」独立开关，能力收敛到订阅状态。每次定时同步盘口后，对**已有赛前盘口的未开赛场次**按后端持久化的**比赛所在地日历日**各自做单选排序：独赢主/客胜与真实亚洲让球胜/负共同入池；冻结历史按玩法做时间留出 Platt 校准（最近 20% 只验证，Brier 与 log-loss 未同时改善则保持原概率）；`|让球线| ≤ 0.5` 按 1X2 概率计入退半/走水，深盘 AH 概率将高于市场公平概率的差异收缩一半。基础分为 `校准概率 × sqrt(赔率 - 1)`，在命中率与水位回报间折中，最低置信度 40%；EV 只随行审计、不作正值门槛。历史 EMA 与联赛×玩法软权重乘入最终综合分。每天最多 4 场、同一场只取一个玩法；有另一玩法可用时同玩法最多 3 场，另一玩法不足才放宽补满。入选后同比赛日按综合分评级，最高锚定 5 星，其余每档减 0.5 星。自动推荐只用于【比赛】/【赛程】置顶、`[荐]` 与质量星，不点亮关注星、不进入【关注】。拒绝所有多选、平局单选与走水推荐；精确比分只作同源提示。窗口内有多少个有赛日就有多少组推荐，不压成整周 Top4。不做前端轮询或 SSE。**让球胜平负的展示与结算**只在「我的 → 偏好设置」用同一开关切换：默认亚盘；竞彩按绝对值向上取整后结算，不影响日推排序。真源：`recommendation.pipeline`、`recommendation.strategy.risk_adjusted_return_score`、`ah_features.jc_handicap_line`。
...
| 同步后自动推荐 | 独赢主/客胜与真实亚洲让球胜/负共同入池；时间留出校准后按 `概率 × sqrt(净赔率)` 的风险调整回报分叠加历史软权重排序，EV 只审计；每天最多 4 场、同一场只取一个玩法、同玩法通常最多 3 场。自动推荐在【比赛】/【赛程】置顶并显示 `[荐]`/质量星，但不点亮关注星且不进入【关注】；卡片整行展示同源日推三件套 |
