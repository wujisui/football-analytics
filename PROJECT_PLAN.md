...
已订阅范围：每天 11:00（或当天尚未成功时的「立即同步」）跑唯一完整批次。赛程在本地始终保留今天起 8 天，但每天只增量拉窗口末端新一天，不重复请求重叠的未来 7 天；近 4 天赛果仍按日回写。盘口与详情只处理今天、明天：明天首次可用盘口写初盘，第二天成为今天后更新即时盘；详情预拉只覆盖今天、明天热门未开赛缺包，打开详情仍可按需补整包。11:55 及 14/16/18/20/22/00/02 只刷新今天未开赛热门盘口并重算日推；04/06/08/10 由默认开启的独立开关控制。当天完整批次成功后「立即同步」禁用；「只更新赛果」仍可按近 4 天回写比分
...
| 盘口轻刷批次 | 只刷新今天未开赛热门盘口并重算日推；未订阅仅 22:00，已订阅为 11:55、00/02/14/16/18/20/22，04/06/08/10 可关闭 |
