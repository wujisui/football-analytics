...
未订阅范围：官方请求固定为「00:05 UTC（北京时间 08:05）当天赛程 + 11:00 昨天赛果/今天比赛/热门盘口 + 22:00 今天热门盘口」。08:05 只多一次全球 `fixtures?date=` 请求，**不拉盘口、积分榜或详情**；11:00 跳过积分榜与详情预拉、不请求未来；详情点击完全只读本地。当天完整批次尚未成功时可由管理员「立即同步」补跑，成功后按钮禁用；管理员另可点「只更新赛果」按日回写终场比分（未订阅含昨天与今天），不占用完整批次当日额度、不拉盘口；单场盘口只能在详情「我的预测」点击「更新盘口」
