...
```
开赛前：赛程同步时初始化；开赛前 30 分钟每场最后强制刷新一次盘口 → 写入本地并重算推荐
...
| 时点 | 盘口与推荐刷新策略 |
|---|---|
| 赛程同步 | 只补缺失盘口；首次可用盘口冻为初盘并完成初始分析 |
| 开赛前 30 分钟 | 每场最多强制刷新一次临场盘口，并据此重算本地推荐 |
...
- [x] `final_odds_update`：每 5 分钟扫描临场窗口；开赛前 30 分钟内每场只刷新一次盘口并重算推荐
...
| 步骤 | 交付 | 状态 / 门禁 |
|------|------|-------------|
| AH-0 | 标签结算函数 + 单元测试（含 -0.5 / +0.5 / 整球走水） | ✅ 已完成 |
...
5. 开赛后预测快照只读冻结值；详情展示包缺数据时可按需补拉，但不轮询比分  
