...
```
开赛前：定时同步批次只为管理员勾选的热门联赛刷新盘口，首次可用盘口冻结为初盘。管理员「订阅」开关是官方套餐边界：未订阅默认每天 **00:05 UTC / 08:05 北京时间**只同步当天赛程、**07:00** 昨天和今天赛果、**11:00** 完整批次 + **22:00** 盘口轻刷；已订阅（至少 Pro、日配额 7500）在 **07:00** 回写昨天和今天赛果、**11:00** 跑每日定时完整批次，其余盘口批次只刷新当天未开赛热门场次并重算日推
...
未订阅范围：官方请求固定为「00:05 UTC（北京时间 08:05）当天赛程 + 07:00/11:00 昨天与今天赛果 + 11:00 今天比赛/热门盘口 + 22:00 今天热门盘口」。08:05 只多一次全球 `fixtures?date=` 请求，**不拉盘口、积分榜或详情**；07:00 回写昨天和今天终场比分；11:00 跳过积分榜与详情预拉、不请求未来；普通用户详情点击完全只读本地。管理员「立即同步」随订阅关闭而禁用；管理员仍可点「只更新赛果」回写昨天和今天终场比分（与 07:00 定时范围相同），不拉盘口；也可在运维页显式点「批量更新盘口」，仅接收前端【比赛】当前筛选已勾选联赛所对应的未开赛场次名单，已有盘口刷新即时盘、缺盘口补齐，不拉赛程、赛果、积分榜或详情；还可在任意未开赛场次详情「我的预测」点击「更新盘口」，不受热门联赛范围限制
...
| 11:00 完整批次 | 未订阅处理昨天和今天赛果、今天赛程/热门盘口；已订阅处理昨天和今天赛果、8 天赛程滑动增量、今天及未来三天热门盘口、今天/明天详情及积分榜；首次可用盘口冻为初盘；随后训练、日推与清理 |
| 盘口轻刷批次 | 只刷新今天未开赛热门盘口并重算日推；未订阅仅 22:00；已订阅在完整默认方案与完整密刷方案间二选一，04/06/08/10 早间时刻可独立叠加 |
| 管理员「批量更新盘口」 | 只处理前端明确传入的【比赛】当前筛选中，`match_day` 等于今天且 `leagues.is_hot=true` 的未开赛场次；明天及非热门赛事即使在列表或请求名单中也由后端排除。已有盘口刷新即时盘、缺盘口补齐；不拉赛程、赛果、积分榜或详情，完成后重算日推 |
| 同步后自动推荐 | 独赢主/客胜与真实亚洲让球胜/负共同入池；时间留出校准后按 `概率 × sqrt(净赔率)` 的风险调整回报分叠加历史软权重排序，EV 只审计；每天最多 4 场、同一场只取一个玩法，玩法之间不设配额。自动推荐在【比赛】/【赛程】置顶并显示 `[荐]`/质量星，但不点亮关注星且不进入【关注】；卡片整行展示同源日推三件套 |
