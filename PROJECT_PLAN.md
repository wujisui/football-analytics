...
开赛后：预测快照审计字段冻结；详情展示包可在用户点开时按需补缺并落库
赛果：六个固定同步批次回写终场比分（非滚球）
前端：始终只调本项目后端。列表页、F5、下拉刷新、切换筛选只读本地库；进入详情走 `/fixtures/{id}/analysis`（后端本地优先，缺包时可打官方）。定时批次按 `SCHEDULER_TIMEZONE` 每天 **00:00、06:00、11:00、16:00、19:00、22:00** 同步赛程、盘口、赛果，并对窗口内**有比赛的联赛**按「联赛+赛季」各拉一次积分榜（同一自然日已拉过则跳过）；列表排名只读本地联赛积分榜快照。不做前端轮询或 SSE；不做定时全量详情预拉（默认关，管理员可在「我的 → 管理员设置」开启并落库）
...
| 时点 | 盘口与推荐刷新策略 |
|---|---|
| 六个固定同步批次 | 全量赛程、一级联赛已有/缺失盘口与赛果统一更新；首次可用盘口冻为初盘；窗口内有赛联赛积分榜按日各拉一次 |
| 勾选其他联赛 | 只筛选数据库中已有赛程，不触发官方盘口请求 |
| 用户打开详情 | 本地缺展示包时按需补拉阵容/伤病/交锋/近况/简报等并落库；积分榜优先读联赛快照，缺时再补拉并回写快照 |
| 已开赛或已结束 | **预测快照审计字段不再刷新**；展示包仍可按点击补缺；赛果随固定批次回写 |

列表归属：计算器是唯一赛前列表，只展示 **未开赛（pending）**；预测卡片内附带让球主盘一行（多盘口悬停展开）；开赛后（live）与完场一并进入 **赛果** 日列表（不做滚球刷新，状态随本地库同步更新）。
