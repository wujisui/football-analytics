...
```
开赛前：定时同步批次只为管理员勾选的热门联赛刷新盘口，首次可用盘口冻结为初盘（默认免费配额模式每天 **11:00** 全量 + **22:00** 盘口轻刷，且只拉今天；关闭后恢复未来窗口与六个整点）
开赛后：预测快照审计字段冻结；已有冻结快照的场次详情只读本地展示包，官方只补比分与状态（开赛 12 小时内 60 秒缓存；此后到 7 天内仍未结算的按 30 分钟缓存补一次终场，不轮询）；按点击补缺展示包只发生在未开赛或从未分析过的场次
赛果：免费配额批次优先回写昨天终场比分（非滚球），再拉今天赛程与盘口
免费配额范围：官方请求固定为「昨天赛果 + 今天比赛/热门盘口」，**跳过积分榜与全部详情获取**（防止逐联赛/逐场请求耗光当日配额），不请求未来比赛；详情点击完全只读本地，连缺失盘口也不按点击补拉；管理员已开启的定时全量详情设置在免费配额期间暂停。关闭免费配额后才恢复配置的未来窗口、积分榜与热门联赛全量详情按需获取
...
- [x] `scheduled_fixtures_sync`：默认免费配额模式每日 11:00 同步昨天赛果与今天赛程/盘口，22:00 轻量刷新盘口并重算每日推荐；关闭后恢复六个整点与未来窗口；首次盘口冻为初盘，新增标签后条件自动 train
