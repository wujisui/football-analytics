...
前端：始终只调本项目后端。列表页、F5、下拉刷新、切换筛选只读本地库；进入详情走 `/fixtures/{id}/analysis`（后端本地优先，缺包时可打官方）。定时批次按 `SCHEDULER_TIMEZONE` 每天 **00:00、06:00、11:00、16:00、19:00、22:00** 同步赛程、盘口、赛果，并对窗口内**有比赛的联赛**按「联赛+赛季」各拉一次积分榜（同一自然日已拉过则跳过）；列表排名只读本地联赛积分榜快照。每次定时同步盘口后，从热门联赛（`config/leagues.json`）未开赛场次中按**单选**算法推荐置信度挑出最多 4 场写入「关注」（`source=auto`，只替换自动关注，不碰手动关注；拒绝胜/平、让胜/负等多选）。不做前端轮询或 SSE；不做定时全量详情预拉（默认关，管理员可在「我的 → 管理员设置」开启并落库）
...
| 同步后自动关注 | 热门联赛未开赛场次按 1X2/让球/大小/双进/比分**单选**置信度取 Top4，写入关注（`source=auto`） |
