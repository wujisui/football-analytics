...
联赛范围：可勾选目录、分类与热门状态以数据库 `leagues` / `league_categories` 为唯一真源；`backend/config/leagues.json` 只在首次建库时导入种子，既有种子联赛标记为系统保护项，管理员不能删除。管理员可在「我的 → 管理员设置 → 热门联赛」新增分类、按官方稳定 `league.id` 新增联赛、修改中文名/国家/分类与勾选热门；后台新增联赛可改正写错的官方 ID（错误 ID 下已拉赛程会丢弃），也可在管理员密码二次确认后连同该联赛全部赛程/盘口/特征/推荐等历史数据删除，之后允许重新添加。非热门目录联赛仍入库赛程但不定时补盘；删除的非保护联赛退出目录并停止新赛程准入。未勾选的目录联赛及额外白名单赛事进入「其他」。`competition_scope.py` 只保留目录外已核对的各国顶级联赛、主要足球地区高级别杯赛及友谊赛；青年、预备队、乙级及以下和低级别杯赛不入库。筛选弹窗里热门默认勾选，切换日期后仍默认勾选（只有用户点过确认的选择才落 localStorage，加载时推导的勾选不落库）；筛选勾选只影响本地显示。已开赛详情只补比分与状态。
