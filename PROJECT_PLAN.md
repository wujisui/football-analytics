...
- [x] `clean_old_data`：每天 11:00 全量同步（及管理员「立即同步」）之后执行；物理删除「永远无法结算」（取消 / 完场无比分 / 延期超 1 天）与「已完场且无赛前 1X2 盘口」（含有预测但无盘口的无依据预测）的场次及关联数据；清理空联赛行、孤立球队与过期展示分析/日志
...
| 价值数据清理 | ✅ | 每天 11:00 全量同步后跑 `clean_old_data`：删「永远无法结算」（取消/无比分/延期过期，含有赛前包的）与「完场且无赛前盘口」（有预测也删，无盘口的预测没有依据）的场次；未开赛赛程与刚延期场次保留 |
