...
```
开赛前：定时同步批次只为管理员勾选的热门联赛刷新盘口，首次可用盘口冻结为初盘。管理员「订阅」开关是官方套餐边界：未订阅默认每天 **00:05 UTC / 08:05 北京时间**只同步当天赛程、**11:00** 完整批次 + **22:00** 盘口轻刷；已订阅（至少 Pro、日配额 7500）在 **11:00** 跑当天唯一完整批次，其余盘口批次只刷新当天未开赛热门场次并重算日推
...
未订阅范围：官方请求固定为「00:05 UTC（北京时间 08:05）当天赛程 + 11:00 昨天赛果/今天比赛/热门盘口 + 22:00 今天热门盘口」。08:05 只多一次全球 `fixtures?date=` 请求，**不拉盘口、积分榜或详情**；11:00 跳过积分榜与详情预拉、不请求未来；普通用户详情点击完全只读本地。当天完整批次尚未成功时可由管理员「立即同步」补跑，成功后按钮禁用；管理员另可点「只更新赛果」按日回写终场比分（未订阅含昨天与今天），不占用完整批次当日额度、不拉盘口；管理员可在运维页显式点「补齐比赛盘口」，只对当前【比赛】默认两个当地比赛日窗口中尚未开赛且缺少完整 1X2 盘口的本地场次逐场请求盘口，不拉赛程、赛果、积分榜或详情；也可在任意未开赛场次详情「我的预测」点击「更新盘口」，不受热门联赛范围限制
已订阅范围：每天 11:00（或当天尚未成功时的「立即同步」）跑唯一完整批次。赛程在本地始终保留今天起 8 天，但每天只增量拉窗口末端新一天，不重复请求重叠的未来 7 天；近 4 天赛果仍按日回写。盘口与详情只处理今天、明天：明天首次可用盘口写初盘，第二天成为今天后更新即时盘；详情预拉只覆盖今天、明天热门未开赛缺包，打开详情仍可按需补整包。盘口轻刷有两套独立完整方案：默认方案为 02:00、11:55、14:00、16:00、18:00、20:00、21:00、21:30、22:00、22:30、23:00、23:30、00:00；密刷方案为 02:00、11:55、14:00、16:00、16:55、17:25、17:55、18:25、18:55、19:25、19:55、20:25、20:55、21:25、21:55、22:25、22:55、23:25、23:55、00:25、00:55、01:25、01:55。运维开关只在两套方案间切换；04/06/08/10 由默认开启的独立早间开关叠加到任一方案。当天完整批次成功后「立即同步」禁用；「只更新赛果」仍可按近 4 天回写比分
前端：始终只调本项目后端。列表页、F5、下拉刷新、切换筛选只读本地库；进入详情走 `/fixtures/{id}/analysis`。未订阅时详情完全只读本地公共数据；已订阅时热门联赛缺包可按需打官方并落库复用。已移除「定时全量获取详情」独立开关，能力收敛到订阅状态。每次定时同步盘口后，对**已有赛前盘口的未开赛场次**按后端持久化的**比赛所在地日历日**各自做**单选价值排序**：先用冻结历史按玩法做时间留出 Platt 概率校准（最近 20% 只验证，Brier 与 log-loss 未同时改善的玩法保持原概率），再以校准概率计算期望收益；历史命中反馈只参与综合分调整、不设置历史分位淘汰门槛，每天按综合分最多选 4 场写入自动推荐池（`source=auto`，只替换自动推荐）。入选后在同一比赛日内部按综合分评级，最高质量锚定 5 星，其余每降一个得分档位减 0.5 星，同分同星。自动推荐只用于【比赛】/【赛程】置顶、`[荐]` 与质量星展示，不点亮关注星，也不进入【关注】；【关注】只展示用户点亮星标并经收藏接口写入数据库的 `source=manual` 记录。校准只改变排序、不减少每日数量；拒绝胜/平、让胜/负等多选；精确比分因无独立校准概率且历史命中率低，只作详情提示，不参与自动主推；窗口内有多少个有赛日就有多少组推荐，禁止把整周场次压成全局 Top4。不做前端轮询或 SSE。**让球胜平负的展示与结算**只在「我的 → 主题与玩法」用同一开关切换：默认**亚盘**（让胜/让负；整数盘打成让球平走水，四分盘出赢半/输半）；**竞彩**先把盘口按绝对值向上取整成整数（-0.25/-0.5/-0.75 → -1，-1.25~-1.75 → -2）再判让胜/让平/让负，不出半结果，展示的盘口同步显示取整后的线。口径只作用于让球胜平负，大小球、双进等其余玩法一律按亚盘拆盘。只改本机展示与结算，不回写冻结预测或已存方案；机器学习与每日推荐激励仍按产品默认亚盘。真源：`ah_features.jc_handicap_line`。
...
| 时点 | 盘口与推荐刷新策略 |
|---|---|
| 11:00 完整批次 | 未订阅处理昨天赛果、今天赛程/热门盘口；已订阅处理近 4 天赛果、8 天赛程滑动增量、今天/明天热门盘口/详情及积分榜；首次可用盘口冻为初盘；随后训练、日推与清理 |
| 盘口轻刷批次 | 只刷新今天未开赛热门盘口并重算日推；未订阅仅 22:00；已订阅在完整默认方案与完整密刷方案间二选一，04/06/08/10 早间时刻可独立叠加 |
| 管理员「补齐比赛盘口」 | 只扫描当前【比赛】默认两个当地比赛日窗口，逐场补拉尚未开赛且缺少完整 1X2 的本地比赛（单次最多 250 场）；不限热门勾选，不拉赛程、赛果、积分榜或详情，完成后重算日推 |
