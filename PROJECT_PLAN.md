...
| 赛前概率分析           | 🟡 MVP  | 近况启发式出概率；推荐/让球/比分为规则层（`prediction.py`）；**ML 入模待做** |
| 业务 API           | ✅ MVP   | leagues / today / sync（含按日批量赔率） / results / analysis / admin |
| 定时任务             | ✅ 完成    | 每日初始化、赛前窗口更新、赛果回写、清理                             |
...
- [x] 今日列表分析不拉完整包（省配额）；**赔率随 sync 按日批量入库**；详情只读本地盘口，不再 `/odds?fixture=`
- [ ] **ML 概率模型**：用历史赛果 + 盘口/近况/交锋特征学习胜平负概率，逐步替代手写启发式与手工推荐规则
...
| 需求     | 现状                                       | 缺口                        |
|--------|------------------------------------------|---------------------------|
| 赛前赔率   | sync 按日批量 `/odds?date=` 写入本地；详情只读库 | 未同步过则空态；勿在详情兜底打单场 odds |
...
| 球队统计   | 已拉取                                      | 未进入公式，未展示                 |
| ML 推荐  | 无；`prediction.py` 为显式规则                    | 用赛果闭环训练概率/倾向，减少手工加规则     |
...
| 分析模型升级             | 启发式 MVP；下一阶段：特征落库 + 赛果标签训练概率模型 |
