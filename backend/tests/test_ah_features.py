...
class AhFeaturesTests(unittest.TestCase):
    ...
    def test_structural_pick_uses_devig_main_handicap_prices(self) -> None:
        odds = {
            "available": True,
            "asian_handicap": {"line": "-0.5", "home": 2.06, "away": 1.84},
        }
        pred = _structural_pick(odds)
    ...
    def test_integer_line_draw_always_walks(self) -> None:
    ...
    def test_display_lean_keeps_the_real_asian_line(self) -> None:
        self.assertEqual(display_bettable_handicap_lean("让胜(-0.5)"), "让胜(-0.5)")
        self.assertEqual(display_bettable_handicap_lean("让负(+0.25)"), "让负(+0.25)")

    def test_display_lean_drops_the_non_bettable_draw(self) -> None:
        self.assertEqual(display_bettable_handicap_lean("让负/平(-1)"), "让负(-1)")
        self.assertEqual(display_bettable_handicap_lean("让平(-1)"), "走水(-1)")
