...
4. 日推每场只取一个玩法：AH 的校准概率扣除方向惩罚后仍达到 50% 才保留，否则降到大小球与双进并按调整后概率竞争；没有 AH 盘时，独赢只作两者都不可用后的末级兜底。四种玩法都在 Top-N 截取前由 `recommendation.pipeline._consistent_bundle` 配出同源胜负方向、真实让球行与参考比分；无法表达的候选跳过并由同场降级层或后续场次补位，不改盘口线。三件套写 `favorite_fixtures` / `auto_pick_snapshots`，不回写 `pre_match_data`
