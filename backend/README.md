...
**其他赛事**：官方按日响应在入库前由 `app/services/competition_scope.py` 的稳定 ID 白名单过滤，只保留各国顶级联赛、主要足球地区高级别杯赛及俱乐部/国际友谊赛；青年、预备队、乙级及以下和低级别杯赛不落库。白名单内但不在 `leagues.json` 的赛事自动进入筛选框（`extra` / 其他赛事）；勾选只影响本地显示，不触发官方请求。新增其他赛事先核对官方 ID，再更新唯一白名单。

API-Sports 没有跨国家统一可靠的“第几级联赛”字段，因此热门目录与其他赛事白名单都按稳定 `league.id` 人工维护。`leagues.json` 条目字段：`name`（中文显示名）、`id`（API-Sports）、`country`；可选 `season`（如世界杯）、`region`（仅注释用，程序忽略）。修改后需**重启后端**。
...
| `clean_old_data`       | 每天 11:00 全量同步之后（管理员「立即同步」也会带上） | 物理删除「永远无法结算」（取消，或开赛超 1 天仍无比分的完场/延期/pending/live）与「完场且无赛前盘口」（含有预测但无盘口的无依据预测）的场次、空联赛行与孤立球队；超过 `CLEANUP_DAYS` 的赛前包只清详情 JSON，长期保留冻结推荐、概率、盘口与训练特征，并清理过期日志 |
