...
| POST | `/admin/tasks/trigger` | 手动触发任务，含完整同步、赛果回写、`prematch_missing_odds_sync`（当前比赛列表缺盘补齐）、清理与训练 |
...
| `scheduled_fixtures_sync` | 11:00 每日唯一完整批次；其余为盘口轻刷 | 未订阅另跑 22:00；已订阅在完整默认方案和完整密刷方案间二选一，04/06/08/10 可独立叠加。轻刷只处理今天未开赛热门盘口并重算日推 |
...
未订阅固定 **08:05** 当天赛程、**11:00** 完整批次、**22:00** 盘口轻刷，跳过积分榜与详情预拉、不请求未来。已订阅的 11:00 完整批次回写近 4 天赛果，赛程保留 8 天但只拉缺失的未来日期（正常每天新增末端一天），盘口与详情只处理今天、明天；任一定时、补齐或管理员单场刷新路径首次成功获取的可用盘口都冻结为初盘，后续刷新即时盘（整份盘口同庄；初盘可由次级庄兜底，主庄开盘后按主庄首盘替换）。盘口轻刷有两套完整方案：默认方案为 02:00、11:55、14:00、16:00、18:00、20:00、21:00 至 00:00 每半小时；密刷方案为 02:00、11:55、14:00、16:00、16:55、17:25 … 01:55；04/06/08/10 可独立叠加。当天完整批次成功后「立即同步」禁用。管理员可另点「只更新赛果」按日回写终场比分（不占用完整批次当日额度），或点「补齐比赛盘口」仅扫描【比赛】默认两个当地比赛日窗口并逐场补拉缺少完整 1X2 的未开赛场次，不拉赛程、赛果、积分榜或详情。详情页「更新盘口」仅管理员可见且接口强制校验管理员权限，可更新任意未开赛场次，不受热门联赛范围限制。列表 / F5 / 筛选始终只读本地；无公开列表 sync、无 SSE、无轮询。
手动：`python manage.py trigger-task --name scheduled_fixtures_sync`（完整批次）、`--name scheduled_results_sync`（只回写赛果）或 `--name prematch_missing_odds_sync`（比赛列表缺盘补齐）。
