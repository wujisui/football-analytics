...
| `scheduled_results_sync` | 每天 **07:00**；管理员手动「只更新赛果」 | 回写调度时区昨天和今天的终场比分（例如周六回写周五晚场，下午完场当天可补），不拉前几天、不拉盘口/赛程/详情 |
| `scheduled_fixtures_sync` | 11:00 每日定时完整批次；其余为盘口轻刷 | 未订阅另跑 22:00；已订阅在完整默认方案和完整密刷方案间二选一，04/06/08/10 可独立叠加。管理员已订阅时可不限每日次数手动完整同步。轻刷只处理今天仍在【比赛】的目录盘口并重算日推 |
...
未订阅固定 **07:00** 昨天和今天赛果、**08:05** 当天赛程、**11:00** 完整批次、**22:00** 盘口轻刷，跳过积分榜与详情预拉、不请求未来。已订阅的 07:00 与 11:00 完整批次都回写昨天和今天赛果，赛程保留 8 天但只拉缺失的未来日期。盘口联赛范围是「热门联赛」配置页中的全部目录联赛（`leagues.is_catalog=true`），不受 `is_hot` 勾选影响；未来三天只补缺盘并冻初盘，不刷新已有盘，今天仍在【比赛】的场次刷新即时盘。盘口轻刷有两套完整方案：默认方案为 02:00、11:55、14:00、16:00、18:00、20:00、21:00 至 00:00 每半小时；密刷方案为 02:00、11:55、14:00、16:00、16:55、17:25 … 01:55；04/06/08/10 可独立叠加。管理员「立即同步」仅已订阅时可用且不限每日次数，每次仍消耗官方配额并受请求锁限制。管理员可另点「只更新赛果」，或点「批量更新盘口」处理【比赛】当前筛选中比赛日为今天的目录场次。详情页「更新盘口」仅管理员可见，且只在今天的目录【比赛】显示；后端使用同一边界强制校验。列表 / F5 / 筛选始终只读本地；无公开列表 sync、无 SSE、无轮询。
