...
4. 未过门禁时**保留**盘口/启发式 lean，不覆盖成「待分析」；**无赛前 1X2 盘口时胜平负与比分留白，其余按「让球 → 大小 → 双进」逐级兜底**（真源 `prediction._leans_without_1x2_market`）——盘口仍是全部玩法的唯一依据，哪一项有报价就给哪一项的倾向，近况模型概率不构成依据，不拿它顶替缺失的盘口
...
2. 固定同步批次回写赛果并打 `ah_label`；样本 ≥ `ML_AH_MIN_TRAIN_SAMPLES`（默认 80）且有新增 → 自动训练；最新 20%（至少 20 条）作时间验证，模型 log-loss 与 Brier 必须同时优于 `ah_implied_cover` 才可部署
3. 分析器以 AH 主盘去水概率为基线；合格模型只取「盘口 + 50% 模型偏差」作保守修正并决定方向，未过门禁则仍取盘口较高侧，同价显示持平；不从 1X2 推荐或参考比分搬方向。相同修正概率写入 `match_features` 供深盘日推，日推不再重复收缩
4. 日推核心层只在独赢主胜 / 客胜与真实让球胜 / 负里单选（`strategy.DAILY_PICK_OUTCOMES`，平局不进候选）；核心不足 4 场时再按 **大小球 → 双进** 降级补位，次级玩法不能凭分数越过上一层候选。四种玩法都在 Top-N 截取前过 `recommendation.consistency.validate_pick_consistency`，以日推的伴随胜负方向加真实让球线，配出不会输的让球侧与按主推玩法重算的比分候选；BTTS 缺大小盘时才用 2.5 中性线生成末级兜底比分。无法表达的候选跳过并由同场下一层玩法或后续场次补位，不改盘口线。三件套写 `favorite_fixtures` / `auto_pick_snapshots`，不回写 `pre_match_data`
5. 详情盘口解释由 `market_analysis.py` 统一生成：使用初盘 / 中盘 / 临场 / 即时盘并按采集时间去重，只比较同庄家、同玩法、同档位的去水概率；分别说明主盘升降、同档共振、1X2、大小球、让球主盘去水倾向及计入赢半/输半/走水后的让球期望收益。**输出文案面向普通用户，一律白话**：不出现「去水」「期望」「共振」「庄家」等术语，期望收益写成「平均每投 100 元赚/亏多少」，`让胜(-0.25)` 这类让球标签保留但补一句「也就是买主队（让 0.25 球）」；初盘 / 即时盘、加减号和大小球不再夹注。`ah_predictor` 产出的 `market_note` 同一口径（回归 `test_market_analysis.py`）。中间态来自已有盘口刷新，不增加官方请求；前端不维护平行判断
...
| POST | `/fixtures/{fixture_id}/odds/refresh` | 管理员详情「更新盘口」：本场即时盘 + 本场算法推荐；不重算日推 |
...
| POST | `/admin/tasks/trigger` | 手动触发任务，含同步数据、更新赛果、`prematch_odds_sync`（运维「更新盘口」）、清理与训练 |
| GET  | `/admin/settings/subscription` | 读取订阅、密刷、当天完整批次、官方剩余用量，以及上次同步（时刻 / 批次类型 / 本次消耗的官方请求数） |
| PATCH | `/admin/settings/subscription` | 写入订阅状态并立刻重排 cron |
| PATCH | `/admin/settings/subscription-dense-odds` | 控制已订阅全天每 30 分钟盘口密刷 |
...
前端「我的 → 管理员设置」可配置官方 Key、切换订阅及密刷，并用登录密码一键清空比赛历史；「管理员设置 → 热门联赛」管理数据库目录、分类与定时拉盘范围。密钥见 [docs/API_SPORTS_KEYS.md](../docs/API_SPORTS_KEYS.md)；清空见 [docs/RESET_MATCH_HISTORY.md](../docs/RESET_MATCH_HISTORY.md)。

### 常用联赛 ID

名称与官方 ID 对照见 [docs/LEAGUE_ID_NAMES.md](../docs/LEAGUE_ID_NAMES.md)（本机库快照，查阅用）。

**目录真源**：数据库 `leagues` / `league_categories` 保存可勾选目录、分类、热门状态与保护标记。`config/leagues.json`（或 `LEAGUES_JSON`）只在首次建库时导入种子。系统保护联赛仅五大联赛、荷甲、葡超、欧冠、中超，不可通过 UI/API 删除、不可改官方 ID。管理员可直接新增分类和联赛、修改中文名/国家/分类与热门状态，无需重启；其余目录联赛可改正官方 ID（错误 ID 下的赛程会丢弃），也可在密码确认后连同该联赛历史删除并重新添加。默认热门为五大联赛、欧冠/欧罗巴/欧协联、中超/日职联/韩K联。
...
| 任务                     | 触发规则      | 作用                                      |
|------------------------|-----------|-----------------------------------------|
| `fixture_rollover` | 未订阅或已订阅但关闭密刷时每天 **00:05 UTC / 北京时间 08:05** | 仅用 1 次全球 `fixtures?date=` 请求同步官方当天赛程，覆盖美洲北京时间早场；不拉盘口、积分榜、详情或未来日期 |
| `scheduled_results_sync` | 每天 **07:00**；运维「更新赛果」 | 回写【比赛】当前比赛日的昨天和今天终场比分，不拉前几天、不拉盘口/赛程/详情 |
| `scheduled_fixtures_sync` | 10:55 每日定时完整批次；其余为盘口轻刷 | 未订阅或已订阅但关闭密刷时另跑 22:00；已订阅并开启密刷时从 11:25 起每 30 分钟全天循环，10:55 接在完整批次后再轻刷。管理员已订阅时可不限每日次数手动完整同步。轻刷只处理今天仍在【比赛】的目录盘口并重算日推 |
| `clean_old_data`       | 每天 10:55 全量同步之后（管理员「立即同步」也会带上） | 物理删除「永远无法结算」（取消，或开赛超 1 天仍无比分的完场/延期/pending/live）与「完场且无赛前盘口」（含有预测但无盘口的无依据预测）的场次、空联赛行与孤立球队；**「完场且无赛前盘口」要等开赛超过 `RESULTS_BROWSABLE_DAYS`（7 天，对齐【赛程】日期条可选范围）才删**——后端漏跑期间的比赛日只能靠赛果回填，而回填走全球按日接口只带比分不带盘口，立刻删会让那几天在赛果页永久空白，且每次同步都重新拉一遍再删，白耗官方配额；超过 `CLEANUP_DAYS` 的赛前包只清详情 JSON，长期保留冻结推荐、概率、盘口与训练特征，并清理过期日志 |

时区由 `SCHEDULER_TIMEZONE` 控制（默认 `Asia/Shanghai`）。  
未订阅固定 **07:00** 昨天和今天赛果、**08:05** 当天赛程、**10:55** 完整批次、**22:00** 盘口轻刷，跳过积分榜与详情预拉、不请求未来。打开订阅会自动开启密刷，关闭订阅会同时关闭并禁用密刷；已订阅时可手动关闭密刷。已订阅的 07:00 与 10:55 完整批次都回写昨天和今天赛果，赛程保留 8 天但只拉缺失的未来日期。盘口联赛范围是「热门联赛」配置页中的全部目录联赛（`leagues.is_catalog=true`），不受 `is_hot` 勾选影响；明天只补缺盘并冻初盘，不刷新已有盘，今天仍在【比赛】的场次刷新即时盘。密刷从 11:25 起按每小时 25/55 分全天循环；10:55 完整批次结束后再执行同刻密刷。关闭密刷后使用 07:00、08:05、10:55、22:00 时刻表，但 10:55 保持已订阅批次语义且「立即同步」仍可用。已订阅时管理员可点「更新赛果」或运维「更新盘口」（【比赛】当前筛选中比赛日为今天的目录场次，批量完成后重算日推）。详情页「更新盘口」仅已订阅管理员可见，且只在今天的目录【比赛】显示；后端使用同一边界强制校验，成功后只重算本场算法推荐、不重排日推。未订阅时「立即同步」「更新赛果」「更新盘口」全部禁用。列表 / F5 / 筛选始终只读本地；无公开列表 sync、无 SSE、无轮询。
前端不会轮询比赛列表或官方 API；仅【比赛】/【赛果】在前台且未息屏时每 30 秒读一次本地 `/fixtures/data-revision`，息屏或离开这两页即停。版本变化或运维操作成功后只刷新当前页的比赛、赛果/赛程和推荐缓存。

手动 CLI：`python manage.py trigger-task --name scheduled_fixtures_sync`（同步数据）或 `--name scheduled_results_sync`（更新赛果）；运维「更新盘口」必须由运维页提交当前【比赛】场次名单。
