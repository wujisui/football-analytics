...
未订阅固定 **07:00** 昨天和今天赛果、**08:05** 当天赛程、**11:00** 完整批次、**22:00** 盘口轻刷，跳过积分榜与详情预拉、不请求未来。已订阅的 07:00 与 11:00 完整批次都回写昨天和今天赛果，赛程保留 8 天但只拉缺失的未来日期（正常每天新增末端一天），盘口覆盖今天及未来三天、详情预拉仍只今天/明天；任一定时、批量或管理员单场刷新路径首次成功获取的可用盘口都冻结为初盘，后续刷新即时盘（整份盘口同庄；初盘可由次级庄兜底，主庄开盘后按主庄首盘替换）。盘口轻刷有两套完整方案：默认方案为 02:00、11:55、14:00、16:00、18:00、20:00、21:00 至 00:00 每半小时；密刷方案为 02:00、11:55、14:00、16:00、16:55、17:25 … 01:55；04/06/08/10 可独立叠加。管理员「立即同步」仅已订阅时可用且不限每日次数，每次仍消耗官方配额并受请求锁限制。管理员可另点「只更新赛果」回写昨天和今天终场比分（与 07:00 定时范围相同），或点「批量更新盘口」只处理【比赛】当前筛选中比赛日为今天的热门未开赛场次；后端按 `fixtures.match_day` 与 `leagues.is_hot` 再过滤，明天及非热门赛事排除。已有盘口刷新即时盘、缺盘口补齐，不拉赛程、赛果、积分榜或详情。详情页「更新盘口」仅管理员可见且接口强制校验管理员权限，可更新任意未开赛场次，不受热门联赛范围限制。列表 / F5 / 筛选始终只读本地；无公开列表 sync、无 SSE、无轮询。
