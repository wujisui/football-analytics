...
| 我的预测  | 算法原始胜平负 + 推荐；盘口：按采集时间去重，初盘早于即时盘才并排两张卡，同一次采集只显示「即时盘」；右侧盘口解释 |
