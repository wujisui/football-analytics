...
左侧预测卡片的「让球」行固定为 6/4 两列：左列是 `让球：` 加主盘三值紧凑排列，右列是校准命中概率，靠右紧凑排列。日推场次显示实际投注那一注的概率，其余场次显示每场参考的概率，因此有无 `[荐]` 的卡片列宽完全一致。真源 `AlgorithmPredictionCard.vue` 与 `RecommendationStrength.vue`；该槽位在标签流里（赛果页 `ResultHitTags`）按内容宽度参与换行，不占位。
