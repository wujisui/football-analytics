...
顶栏导航：桌面为 **比赛** / **赛程** / **关注**，未登录显示 **登录**（弹窗表单），登录后才出现 **我的**；PC 从其他顶栏回到「我的」时打开离开前的子页，手机底栏「我的」始终进个人主页。主题开关只留在「我的 → 偏好设置」，切换即时生效；让球固定按亚盘结算，没有口径开关；手机底栏为 **计算器** / **赛程** / **关注** / **我的**。关注仅走 `/favorites`，不再从「我的」进入；PC 关注页与比赛页同为左侧联赛列表 + 右侧内容区，手机保持单栏；「我的方案」只从「我的」进入。比赛页桌面的预测卡片与计算器卡片左右并排并占满内容区，不为投注详情常驻保留右列；桌面与手机都只在已有选择时显示底部投注摘要，点「展开」从底部 Drawer 查看已选场次，关闭 Drawer 即收起继续浏览。投注详情中的已选项会自动清理：赛程日早于 UTC 今天，或开赛时间已过的场次不再保留（已保存的「我的方案」不受影响）。
...
左侧预测卡片的「让球」行通栏展示 `让球：` 加主盘三值。卡片不显示推荐强度——星级与百分比都试过，两者都要占掉一行宽度，手机上撑乱同列卡片高度，而「本场最看好哪个玩法」已经由 `[荐]` 和绿色标签给出。真源 `AlgorithmPredictionCard.vue`。

玩法选择卡片在桌面与手机统一使用四列：**全场独赢 / 全场让球 / 全场大小 / 全场双进**，每列内纵向排列选项和赔率。格子里标签（主胜 / 客胜 / 让球指数等）用 `--fa-text-strong`，赔率沿用按钮正文色。全场独赢因横排文案，写成 `主胜/1.53` 这种中间斜杠、无 gap、整组居中。每场只允许选中一个格子；重复点击当前格取消，点击本场另一格直接替换。投注摘要固定在当前内容区底部且仅在已选场次不为空时挂载：工具条背景与阴影始终铺满内容区；内部两行在桌面靠左限宽为 `--calc-panel-width`（400px），手机通栏。第一行固定为「过关方式 / 倍数 / 保存方案 / 详情图标 / 清空图标」，手机三个按钮均分剩余宽度铺满整行以保证点击区域，详情展开态再点即收起；清空入口只保留在工具条，不在 Drawer 内重复。已选列表统一放进底部 Drawer，不另设桌面常驻详情列；「奖金算式」与仅手机出现的「保存到相册」放在 Drawer 的 `#header` 插槽里与标题同行，因此 Drawer 不再渲染关闭按钮，收起靠工具条箭头或点遮罩。Drawer 以列表区容器（`.calc-body`）为挂载点，遮罩与滑出范围都限制在内容区内，顶栏、左侧联赛列表与底部投注摘要始终可见；宽度与摘要内容同为 `--calc-panel-width`（手机 100%）且左边缘对齐。高度由 `syncDrawerHeight` 按「标题 + 操作行 + 已选列表」实测，封顶为容器高度的 80%，超出才在列表内滚动——Naive 的 `height: auto` 只作用于抽屉本身，不会把上限传给正文，靠 CSS 百分比会撑出空白或让内容溢出。
...
6. 预测对比之后是双方近 5 场数据对比；再后是官方 `GET /predictions` 中文结论表（建议 / 倾向胜方 / 胜或平 / 大小球 / 预期进球 / 胜平负占比）及官方队际对比表（含泊松分布说明），与本地算法无关
...
| 赛前分析 | 先四阶段盘口表，再预测对比，再数据对比（双方近 5 场：赛 / 总进 / 均进 / 总失 / 均失，两行），最后 API-Sports 官方建议（中文项目/结论表 + 含泊松分布的官方队际对比表）。「赛前结果预测」与「盘口解释」为并排 `section.fa-section`；赛前段未开赛时内联展示胜平负 + 推荐 + 饼图，已开赛改渲染 `AlgorithmPredictionCard`；胜平负三行上方固定标注「下面的百分比来自主盘赔率折算（已扣掉博彩公司抽成），代表市场的看法」；盘口区按初盘 `initial` → 中盘 `mid`（T-6h）→ 临场 `late`（T-1h）→ 即时盘 `current` 顺序展示，各阶段标题使用次级文本色；初盘标题旁只留「首次采集的机构盘口」，其余阶段不写说明；时间用本地 `MM-DD HH:mm`（`formatLocalMonthDayMinute`），不标时区、不加「采集」；按采集时间去重，同一次采集只保留语义更靠后的阶段；右侧展示后端盘口解释。**面向用户的文案一律白话**；真源仍是 `market_analysis.py`，前端不自行推断走势 |
...
后续用户档位与账号权益以根目录 **[docs/AUTH_VIP_QUOTA.md](../docs/AUTH_VIP_QUOTA.md)** 为准；列表不提供「手动刷官方」入口。运维：`/mine/admin` 由订阅状态统一控制完整批次与详情预拉，并可配置全天密刷；打开订阅只解禁密刷开关、不自动开启，关闭订阅同时关闭并禁用。详情「我的预测」的「更新盘口」只向管理员显示，且仅在 `match_day=今天`、仍在【比赛】并属于配置页目录联赛时显示。该按钮只刷新本场盘口并重载本场算法推荐，不重算日推；日推只在运维「更新盘口」或定时/密刷批量盘口之后重排。
