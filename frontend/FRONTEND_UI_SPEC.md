...
后续用户档位与账号权益以根目录 **[docs/AUTH_VIP_QUOTA.md](../docs/AUTH_VIP_QUOTA.md)** 为准；列表不提供「手动刷官方」入口。运维：`/mine/admin` 由订阅状态统一控制完整批次与详情预拉，并可配置早间盘口刷新；详情「我的预测」的「更新盘口」只向管理员显示，且仅在 `match_day=今天`、仍在【比赛】并属于配置页目录联赛时显示。
