...
顶栏导航：桌面为 **计算器** / **赛程**，未登录显示 **登录**（弹窗表单），登录后才出现 **我的**；手机底栏为 **计算器** / **赛程** / **我的**。「收藏」与「我的方案」只从「我的 → 本地数据」进入。计算器桌面为赔率/预测（含让球主盘行，多盘悬停展开）、可选玩法、投注详情三列；手机为场次玩法列表 + 选中后底部固定投注摘要。
