...
顶栏导航：桌面 **即时** / **预测** / **赛程** / **收藏**；手机 **即时** / **计算器** / **赛程** / **收藏**（`/predictions` 在手机为计算器单列：场次玩法列表 + 选中后底部固定投注摘要）。
...
5. 点「**同步赛程**」：先 `POST /api/v1/fixtures/sync?days=7`（绕过 Redis/SQLite 日缓存打官方），成功后再重拉本地列表。有约 90 秒冷却，避免刷爆配额。
