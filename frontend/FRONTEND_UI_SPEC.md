...
顶栏导航：桌面为 **即时** / **计算器** / **收藏** / **赛程**，未登录显示 **登录**（弹窗表单），登录后才出现 **我的**；手机底栏始终为 **即时** / **计算器** / **收藏** / **赛程** / **我的**，「我的」页内提供登录入口（同一登录弹窗）。`/favorites` 为收藏列表页（原抽屉内容）；`/predictions` 在手机为计算器单列：场次玩法列表 + 选中后底部固定投注摘要，「投注详情」弹窗与桌面第三列一致。
...
5. **每个浏览器会话首次打开**自动跑一次 `POST /api/v1/fixtures/sync`（绕过 Redis/SQLite 日缓存打官方）；杀页冷启动若会话仍在则不再自动打官方，只读本地库。列表期间照常展示本地内容，落库后自动重拉本地列表。需要更新时在即时 / 计算器 / 赛程列表**下拉刷新**（列表顶提示文案）。
...
当日列表含 **完场 + 进行中（live）**（开赛后从即时迁入；不做滚球轮询，比分随本地库更新）。命中统计仍只计已完场且可评估的场次。
