...
const syncDetail = computed(() =>
  subscribed.value
    ? '每次都会回写近 4 天赛果、增量补齐 8 天赛程窗口、更新今天及未来三天热门盘口、今天/明天详情、积分榜、训练、日推及清理。不限每日次数，但每次都会消耗官方配额；新增热门联赛后可在这里重新同步。'
...
const subscriptionDetail = computed(() => {
  const clocks = syncTimes.value.length
    ...
    : ''
  if (subscribed.value) {
    return `${clocks}11:00 为每日定时完整批次，其余时刻只刷新今天未开赛热门盘口并重算日推。管理员可不限每日次数手动完整同步；赛程保留 8 天滑动窗口，每天只新增末端一天；盘口覆盖今天及未来三天，详情预拉仍只今天、明天。`
  }
