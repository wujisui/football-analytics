...
const subscriptionDetail = computed(() => {
  ...
  return '未订阅每天 07:00 回写昨天和今天赛果，08:05 只拉当天赛程，10:55 跑昨天/今天赛果与今天赛程/热门盘口，22:00 只刷新今天未开赛热门盘口并重算日推；跳过积分榜与详情预拉，打开详情只读本地。密刷会随订阅关闭并禁用。立即同步、更新盘口、更新赛果均不可用。'
})
...
const denseOddsDetail =
  '订阅只决定本开关能否使用，打开订阅不会自动开启密刷。从 11:25 起按每小时 25 分、55 分持续循环 24 小时，只刷新比赛日为今天且仍未开赛的目录盘口并重算日推。10:55 会先执行完整批次，再执行同刻密刷。手动关闭后采用 07:00、08:05、10:55、22:00 时刻表，但仍保留已订阅完整批次和立即同步能力。'
...
function onSubscriptionToggle(next: boolean) {
  confirmAdminSwitch(
    ...
    next
      ? '会解禁密刷开关（保持当前状态，不自动开启），并启用 8 天增量赛程、积分榜、今天即时盘、明天初盘及今天/明天详情。开关本身不会立即同步。'
