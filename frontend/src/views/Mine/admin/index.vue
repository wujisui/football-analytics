...
function onToggle(next: boolean) {
  ...
  modal.create({
    ...
    autoFocus: false,
    type: 'warning',
    content: `开启后不会立刻拉官方接口。会在下一次定时批次（每批最多 ${detailBudget.value} 场），或你点「立即同步」时为热门联赛未开赛且缺展示包的场次预拉详情（立即同步不限场次）；会额外消耗官方 API 配额，配额耗尽会提前停止。确定开启？`,
    positiveText: '确认开启',
    negativeText: '取消',
    onPositiveClick: () => {
  ...
  try {
    ...
    if (next) {
      if (data.catch_up_started) {
        message.success(
          '已开启免费配额模式：详情完全只读本地；每日 08:05 只同步一次当天赛程，盘口仍仅由 11:00/22:00 批次获取。今日 11:00 已过，正在后台补跑一次同步',
        )
      } else {
        message.success(
          `已开启免费配额模式：08:05 只同步当天赛程；盘口批次：${formatSyncHours(freeQuotaHours.value)}`,
        )
      }
    } else {
      message.success(
...
function onFreeQuotaToggle(next: boolean) {
  if (!next) {
    modal.create({
      ...
      type: 'warning',
      content:
        '关闭后将恢复每天 00:00、06:00、11:00、16:00、19:00、22:00 共 6 次官方同步，并恢复热门联赛详情的全量按需获取；赛程窗口为今天起 8 天，交锋/近况不再限制在 2022–2024。配额消耗会明显增加。确定关闭？',
      positiveText: '确认关闭',
      negativeText: '取消',
      onPositiveClick: () => {
  ...
  modal.create({
    ...
    type: 'warning',
    content:
      '免费配额模式：开启后立刻重排定时任务，每日 08:05 仅用一次请求同步当天赛程，11:00 同步昨天赛果与今天比赛/热门盘口，22:00 再轻量刷新盘口并重算每日推荐；所有详情点击完全只读本地，不补盘口或其它详情，定时全量详情也暂停；跳过积分榜、不拉未来比赛。若今日 11:00 已过会立即补跑一次。确定开启？',
    positiveText: '确认开启',
    negativeText: '取消',
    onPositiveClick: () => {
      ...
      <n-list>
        <n-list-item>
          ...
          <n-thing
            ...
            :description="
              statusText ||
              '立即执行同步；免费配额模式下只拉昨天赛果与今天赛程/盘口，08:05 另以一次请求补当天早场；跳过积分榜、不拉未来比赛。关闭免费配额后拉今天起 8 天赛程，交锋/近况不再夹在 2022–2024'
            "
          />
          <template #suffix>
            <n-button
        ...
        <n-list-item>
          ...
          <n-thing
            ...
            :description="
              freeQuotaSource
                ? `免费配额模式（每日 08:05 当天赛程 + 11:00 全量 + 22:00 热门盘口；08:05 固定只用 1 次请求且不拉盘口；详情完全只读本地，不拉未来）。当前来源：${
                    freeQuotaSource === 'db' ? '管理员覆盖（库）' : '环境变量默认'
                  }；生效整点：${formatSyncHours(freeQuotaHours)}`
                : '免费配额模式（每日 08:05 当天赛程 + 11:00 全量 + 22:00 热门盘口；08:05 固定只用 1 次请求且不拉盘口；详情完全只读本地，不拉未来）；默认开启'
            "
          />
          <template #suffix>
            <n-switch
        ...
        <n-list-item>
          ...
          <n-thing
            ...
            :description="
              freeQuotaEnabled
                ? `免费配额开启中，定时详情暂停；关闭免费配额后恢复此设置（定时批次每批最多 ${detailBudget} 场，「立即同步」不限场次）`
                : source
                ? `当前来源：${source === 'db' ? '管理员覆盖（库）' : '环境变量默认'}；开关只改设置，真正预拉发生在下一次定时批次（每批最多 ${detailBudget} 场）或「立即同步」（热门联赛未开赛缺包，不限场次）`
