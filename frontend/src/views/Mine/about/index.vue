...
const pages: { name: string; detail: string }[] = [
  ...
  },
  {
    name: '赛程',
    detail:
        '按赛程日回看已开赛与完场比赛，完场标注实际比分；统计胜平负、让球、大小球、双方进球与每日推荐各自的命中率，含当日统计与近 30 天走势。让球命中按顶栏当前玩法结算。',
  },
  {
    name: '关注',
    detail:
  ...
  },
  {
    name: '我的',
    detail:
        '账号信息、我的方案（按赛程日回溯方案命中情况）、主题与让球玩法（亚洲盘 / 竞彩三项）以及本页产品说明。',
  },
]
...
const logic: { name: string; detail: string }[] = [
  {
    name: '★ 星标：每个比赛日最多 4 场',
    detail:
      '只从热门联赛里已备好分析包、且尚未开赛的比赛中挑选。系统先用冻结历史按玩法校准预测概率：最近 20% 样本只做时间验证，校准没有同时改善误差的玩法继续使用原概率；再结合赔率价值、每日推荐命中反馈与联赛 × 玩法历史表现调整综合分。每个比赛日仍按综合分挑最多 4 场，不因校准减少数量。每次定时同步后重挑，开赛后的推荐轨迹冻结留档。',
  },
  {
    name: '[荐] 标签：每场只推一个玩法',
    detail:
  ...
  },
  {
    name: '质量提示：相对星级',
    detail:
      '入选后只在同一比赛日的推荐场次内部比较综合分：最高质量的一场为 5 星，其余每降低一个得分档位降 0.5 星，同分同星。星级不再设置样本门槛，关注星标与 [荐] 标签保持统一金色。',
  },
  {
    name: '使用路径',
    detail:
