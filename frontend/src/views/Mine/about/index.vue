...
const logic: { name: string; detail: string }[] = [
  {
    ...
    detail:
      '从未开赛且本地已有盘口的比赛中挑选，不限热门或「其他」。系统先用冻结历史按玩法校准预测概率：最近 20% 样本只做时间验证，校准没有同时改善误差的玩法继续使用原概率；再结合赔率价值、每日推荐命中反馈与联赛 × 玩法历史表现调整综合分。每个比赛日按综合分挑最多 4 场：上限固定，比赛少的日子也不会多推；当天够格的候选不足 4 场时按实际数量给，不凑数。推荐落在「其他」联赛时，该联赛会在【比赛】筛选里自动勾上，不会因为没勾热门而看不到。推荐场次排在【比赛】列表顶部，不自动点亮关注星标；只有你手动点星才会写入个人收藏。每次定时同步后重挑，开赛后的推荐轨迹冻结留档。',
  },
  {
    name: '[荐] 标签：每场只推一个玩法',
    detail:
