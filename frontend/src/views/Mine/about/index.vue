...
const logic: { name: string; detail: string }[] = [
  ...
  },
  {
    name: '[荐] 标签：每场只推一个玩法',
    detail:
        '一场比赛给出胜平负、让球、大小球、双方进球、比分五项预测，但只有一个玩法带 [荐]，代表这场最看好的选项。候选只收单选玩法，胜/平、让胜/负这类多选一律排除；精确比分只作详情提示，不参与主推。让球盘只买命中率过半的那一侧，不会为了水位更高去买对面的低概率侧。因此日推与分析器同向的居多，仅当分析器看好的那个结果本身不到五成时才可能不同向：主胜 45% 已是三者最高，但 -0.5 盘的让负（客队不败）覆盖平和客胜共 55%，此时日推买让负。',
  },
  {
    name: '质量提示：相对星级',
    detail:
