...
const logic: { name: string; detail: string }[] = [
  ...
  },
  {
    name: '[荐] 标签：每场只推一个玩法',
    detail:
        '一场比赛给出胜平负、让球、大小球、双方进球、比分五项预测，但只有一个玩法带 [荐]，代表这场最看好的选项。候选只收单选玩法，胜/平、让胜/负这类多选一律排除；精确比分只作详情提示，不参与主推。分析器的让球方向读取主盘两侧水位，去水后展示市场隐含概率更高的一侧；日推则用冻结历史校准后的条件命中率，同一块盘只保留较高侧，再与独赢候选比较风险调整回报。因此两者通常同向，但当历史校准概率与当前盘口定价分歧时仍可能不同向。',
  },
  {
    name: '质量提示：相对星级',
    detail:
