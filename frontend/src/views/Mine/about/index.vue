...
const logic: { name: string; detail: string }[] = [
  {
    ...
    detail:
      '从未开赛且本地已有盘口的比赛中挑选，不限热门或「其他」。系统先用冻结历史按玩法校准预测概率：最近 20% 样本只做时间验证，校准没有同时改善误差的玩法继续使用原概率；再结合赔率价值、每日推荐命中反馈与联赛 × 玩法历史表现调整综合分。每次重挑时按综合分取最多 4 场，屏幕上同时出现的推荐不会超过 4 场。只有当天未开赛的比赛池不足 6 场，或够格的候选本来就不到 4 个时，才会少于 4 场，不会为了凑数放低标准。推荐落在「其他」联赛时，该联赛会在【比赛】筛选里自动勾上，不会因为没勾热门而看不到。推荐场次排在【比赛】列表顶部，不自动点亮关注星标；只有你手动点星才会写入个人收藏。每次定时同步后重挑：已开赛那场的推荐轨迹冻结留档并让出位置，由还没开赛的下一场补上，因此一个比赛日累计下来的推荐注数会多于 4，【赛程】里的「当日累计推荐」统计的就是这全部注数。',
  },
  {
    name: '[荐] 标签：每场只推一个玩法',
    detail:
