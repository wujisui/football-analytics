...
const pages: { name: string; detail: string }[] = [
  ...
  },
  {
    name: '赛程',
    detail:
        '按赛程日回看已开赛与完场比赛，完场标注实际比分；统计胜平负、让球、大小球、双方进球与每日推荐各自的命中率，含当日统计与近 30 天走势。日推与分析器是两条轨道，卡片头部 [荐] 显示当时冻结的投注项并按它自己结算，与下方五项分析器命中互不影响。让球固定按亚盘结算。',
  },
  {
    name: '关注',
    detail:
...
const logic: { name: string; detail: string }[] = [
  {
    ...
    detail:
      '从未开赛且本地已有盘口的比赛中挑选，不限热门或「其他」。系统先用冻结历史按玩法校准预测概率：最近 20% 样本只做时间验证，校准没有同时改善误差的玩法继续使用原概率；再结合赔率价值、每日推荐命中反馈与联赛 × 玩法历史表现调整综合分。每个比赛日按综合分挑最多 4 场，上限固定，比赛再多也不会多推。只有当天未开赛的比赛池不足 6 场，或够格的候选本来就不到 4 个时，才会少于 4 场，不会为了凑数放低标准。推荐落在「其他」联赛时，该联赛会在【比赛】筛选里自动勾上，不会因为没勾热门而看不到。推荐场次排在【比赛】列表顶部，不自动点亮关注星标；只有你手动点星才会写入个人收藏。每次定时同步后重挑，开赛后的推荐轨迹冻结留档。',
  },
  {
    name: '[荐] 标签：每场只推一个玩法',
    detail:
        '一场比赛给出独赢、让球、大小球、双方进球、比分五项预测，但只有一个玩法带 [荐]，代表这场最看好的选项。候选只收单选玩法，主胜/和局、主-1/客+1 这类多选一律排除；精确比分只作详情提示，不参与主推。分析器让球以主盘去水概率为基线；只有历史模型在最新验证比赛中同时优于盘口的两项概率指标，才会用一半模型偏差保守修正，否则仍按盘口定价。日推再用冻结历史校准命中率并比较风险调整回报，因此两者通常同向，但仍可能因玩法结算与跨场排序而不同。',
  },
  {
    name: '质量提示：相对星级',
    detail:
