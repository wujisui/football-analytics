...
const logic: { name: string; detail: string }[] = [
  {
    name: '★ 星标：每个比赛日最多 4 场',
    detail:
      '从未开赛且本地已有盘口的比赛中挑选，不限热门或「其他」。系统先用冻结历史按玩法校准预测概率：最近 20% 样本只做时间验证，校准没有同时改善误差的玩法继续使用原概率；再结合赔率价值、每日推荐命中反馈与联赛 × 玩法历史表现调整综合分。每个比赛日仍按综合分挑最多 4 场，不因校准减少数量。每次定时同步后重挑，开赛后的推荐轨迹冻结留档。',
  },
  {
    name: '[荐] 标签：每场只推一个玩法',
    detail:
