...
const logic: { name: string; detail: string }[] = [
  ...
  },
  {
    name: '[荐] 标签：每场只推一个玩法',
    detail:
        '所有比赛都有一条“每场参考”：未入选当天前 4 的场次，卡片会把这条参考对应的那个玩法标签标成绿色，表示本场最看好它，只是没好到能进当天前 4。只有进入前 4 的场次带 [荐]；[荐] 指向按“合格让球→大小球/双进→无盘独赢”降级选出的实际投注项。让球与四分之一大小球按全赢、赢半、走水、输半、全输分别计算，不再用简单二元命中率代替真实结算。关注星标仍只表示你的手动收藏。',
  },
  {
    name: '使用路径',
    detail:
