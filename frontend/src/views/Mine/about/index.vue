...
const logic: { name: string; detail: string }[] = [
  {
    name: '每日推荐：命中概率前 4 场',
    detail:
      '概率优先取盘口去水价：自研模型必须在最近一段比赛上同时以两项指标赢过市场，才允许接管它那个玩法的概率，目前四个模型都还没做到，所以推荐跑的是“市场基线 + 分层优选”，不是“我们比庄家更准”。有让球盘时严格按“让球→大小球→双方进球”选择，无让球盘时按“独赢→大小球→双方进球”选择，前一个玩法只在拿不到报价时才降级。方向逆着已明确移动的盘口会被淘汰，逆着弱方向则降权并标成“逆向推荐”。过闸后按“层级 → 命中概率”取当天前 4 场，赔率不参与排序——概率本身就来自赔率，再乘一次净赔率只会系统性地挑最像抛硬币的盘。推荐落在「其他」联赛时，该联赛会在【比赛】筛选里自动勾上。每次批量同步后重挑，已开赛推荐冻结留档，所以「当日累计推荐」仍可能多于屏幕同时展示的 4 场。',
  },
  {
    name: '[荐] 标签：每场只推一个玩法',
    detail:
        '所有比赛都有一条“每场参考”，占用推荐卡片里星级的位置：未入选当天前 4 的场次在那里显示参考方向与命中概率，把概率来源、期望收益和盘口一致性放进悬停说明。悬停里的期望收益按当前口径恒为负，负的幅度就是庄家抽水——它只用于审计，不代表这注没价值，也不代表有价值。只有进入当天前 4 的场次带 [荐] 和星级；[荐] 指向严格玩法顺序选出的实际投注项。让球与四分之一大小球按全赢、赢半、走水、输半、全输分别计算，不再用简单二元命中率代替真实结算。',
  },
  {
    name: '推荐强度：绝对星级',
    detail:
      '星级只给每日推荐，按命中概率的固定区间表示推荐强度，不会因为当天只有几场就强制制造五星。5 星需命中概率至少 65%，4 星至少 57%，3 星至少 51%；概率较低的场次也可能因为当天没有更好的选择而进入前 4，但会如实显示较低星级。关注星标仍只表示你的手动收藏。',
  },
  {
    name: '使用路径',
    detail:
