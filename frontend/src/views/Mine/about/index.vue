...
const pages: { name: string; detail: string }[] = [
  ...
  },
  {
    name: '赛程',
    detail:
        '按赛程日回看已开赛与完场比赛，完场标注实际比分；统计胜平负、让球、大小球、双方进球与每日推荐各自的命中率，含当日统计与近 30 天走势。日推与分析器是两条轨道，卡片上 [荐] 单独成一枚标签，显示当时冻结的投注项并按它自己结算，与旁边五项分析器命中互不影响。让球命中按「我的 → 偏好设置」当前口径结算。',
  },
  {
    name: '关注',
    detail:
...
const logic: { name: string; detail: string }[] = [
  ...
  },
  {
    name: '[荐] 标签：每场只推一个玩法',
    detail:
        '一场比赛给出胜平负、让球、大小球、双方进球、比分五项预测，但只有一个玩法带 [荐]，代表这场最看好的选项。候选只收单选玩法，胜/平、让胜/负这类多选一律排除；精确比分只作详情提示，不参与主推。日推按风险调整回报独立选边，可以和分析器不同向：分析器看主胜时，日推仍可能买 -0.5 盘的让负（客队不败）。',
  },
  {
    name: '质量提示：相对星级',
    detail:
      '入选后只在同一比赛日的推荐场次内部比较综合分：最高质量的一场为 5 星，其余每降低一个得分档位降 0.5 星，同分同星。星级不再设置样本门槛；关注星标只表示你的手动收藏，[荐] 标签保持金色。',
  },
  {
    name: '使用路径',
    detail:
