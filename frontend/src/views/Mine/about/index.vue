...
const logic: { name: string; detail: string }[] = [
  {
    name: '每日推荐：调整后 EV 前 4 场',
    detail:
      '每种玩法只使用自己的模型概率，并经过已通过时间验证的同玩法 Platt 校准后计算 EV；盘口去水概率只用于市场对照，不会冒充模型概率。有让球盘时严格按“让球→大小球→双方进球”选择，无让球盘时按“独赢→大小球→双方进球”选择，前一个玩法只有在模型、校准、赔率或结算数据不足时才会降级。模型方向逆着盘口不会被一刀切掉，而是按强弱扣减 EV 并明确标成“逆向推荐”。系统从每场唯一参考中按调整后 EV 取前 4 场；如果当天可计算 EV 的比赛不足 4 场，会如实少推并记录告警，不拿伪造概率补位。推荐落在「其他」联赛时，该联赛会在【比赛】筛选里自动勾上。每次批量同步后重挑，已开赛推荐冻结留档，所以「当日累计推荐」仍可能多于屏幕同时展示的 4 场。',
  },
  {
    name: '[荐] 标签：每场只推一个玩法',
    detail:
        '所有比赛都有一条“每场参考”，占用推荐卡片里星级的位置：未入选当天前 4 的场次在那里显示参考方向与方向修正后的 EV，把校准概率、原始 EV、盘口一致性和 EV 不可算的原因放进悬停说明。只有进入当天前 4 的场次带 [荐] 和星级；[荐] 指向严格玩法顺序选出的实际投注项。让球与四分之一大小球按全赢、赢半、走水、输半、全输分别计算，不再用简单二元命中率代替真实结算。',
  },
  {
    name: '推荐强度：绝对星级',
    detail:
      '星级只给每日推荐，按调整后 EV 的固定区间表示推荐强度，不会因为当天只有几场就强制制造五星。5 星需调整后 EV 至少 8%，4 星至少 3%，3 星从 0% 起；负值也可进入前 4，但会如实显示较低星级。校准概率单独展示，不与推荐强度混为一谈；关注星标仍只表示你的手动收藏。',
  },
  {
    name: '使用路径',
    detail:
