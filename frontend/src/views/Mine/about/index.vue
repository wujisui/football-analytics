...
const logic: { name: string; detail: string }[] = [
  ...
  },
  {
    name: '[荐] 标签：每场只推一个玩法',
    detail:
        '所有比赛都有一条“每场参考”，占用推荐卡片里星级的位置：未入选当天前 4 的场次在那里显示参考方向与命中概率，把概率来源、期望收益和盘口一致性放进悬停说明。悬停里的期望收益按当前口径恒为负，负的幅度就是庄家抽水——它只用于审计，不代表这注没价值，也不代表有价值。只有进入当天前 4 的场次带 [荐] 和星级；[荐] 指向按“合格让球→大小球/双进→无盘独赢”降级选出的实际投注项。让球与四分之一大小球按全赢、赢半、走水、输半、全输分别计算，不再用简单二元命中率代替真实结算。',
  },
  {
    name: '推荐强度：绝对星级',
    detail:
